 ...
 * Created by Lettle on 2024/4/11.
 * QQ: 1071445082
...
bool esc = FALSE;		            /* 是一个转义扫描码？收到一个转义扫描码时，被置位 */
bool alt_left = FALSE;		    /* 左ALT键状态 */
bool alt_right = FALSE;		    /* 右ALT键状态 */
bool alt = FALSE;		            /* ALT键状态，不分左右 */
bool ctrl_left = FALSE;		    /* 左CTRL键状态 */
bool ctrl_right = FALSE;		    /* 右CTRL键状态 */
bool ctrl = FALSE;		        /* CTRL键状态，不分左右 */
bool shift_left = FALSE;		    /* 左SHIFT键状态 */
bool shift_right = FALSE;         /* 右SHIFT键状态 */
bool shift = FALSE;		        /* SHIFT键状态，不分左右 */
uint8_t locks[NR_CONSOLES];            /* 每个控制台的锁定键状态 */
...
// 映射按键
uint32_t map_key(uint8_t scan_code)
{
    ...
    /* 确定映射行 */
    uint16_t *keys_row = &keymap[scan_code * MAP_COLS];

    /* 确定当前控制台的锁定键状态 */
    uint8_t lock = locks[0];
    bool caps = shift;    /* 如果同时按下了 shift，默认大写状态  */
...
// 执行按键按下/松开处理
uint32_t key_make_break(uint8_t scan_code)
{
    ...
    uint32_t ch = map_key((scan_code &= KEY));
...
{
    /* 1. 从键盘控制器端口获取键盘码 */
    const uint8_t scan_code = in_byte(KEYBOARD_DATA);
